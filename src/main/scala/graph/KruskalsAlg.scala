...
object KruskalsAlg {
  ...
  def apply[V](graph: UndirectedGraph[V]) = {
    ...
    val forest: Set[UndirectedGraph[V]] = (for (v <- graph.vertices) yield UndirectedGraph(Set(v), Set[Edge[V]]())).toSet
